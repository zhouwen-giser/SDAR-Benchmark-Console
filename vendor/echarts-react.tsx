...
function LineChart({ option, onEvents }: Required<Pick<Props, "option">> & Pick<Props, "onEvents">) {
  ...
  return <>{[0, 1, 2, 3].map((line) => <line key={line} x1="38" x2="588" y1={24 + line * 58} y2={24 + line * 58} stroke="#1d3549" />)}{series.map((item: any, seriesIndex: number) => { const points = (item.data ?? []).map((entry: any, index: number) => { const value = Number(typeof entry === "object" ? entry.value : entry) || 0; const x = 42 + index * (540 / Math.max(1, item.data.length - 1)); const y = 224 - ((value - min) / Math.max(1, max - min)) * 190; return { x, y, value, index }; }); return <g key={item.name ?? seriesIndex}><polyline points={points.map((point: any) => `${point.x},${point.y}`).join(" ")} fill="none" stroke={palette[seriesIndex % palette.length]} strokeWidth="2.5" />{points.map((point: any) => <circle key={point.index} cx={point.x} cy={point.y} r="4" fill={palette[seriesIndex % palette.length]} onClick={click(onEvents, { name: labels[point.index], value: point.value, seriesName: item.name })} />)}</g>; })}<g className="chart-axis-labels">{labels.filter((_, index) => index % Math.max(1, Math.ceil(labels.length / 7)) === 0).map((label, index) => <text key={label} x={42 + index * (540 / Math.max(1, Math.min(6, labels.length - 1)))} y="252">{label}</text>)}</g></>;
}
...
export default function ReactECharts({ option = {}, style, onEvents }: Props) {
  ...
  return <div className="echarts-preview" style={style}><svg viewBox="0 0 600 260" preserveAspectRatio="none" role="img" aria-label="数据可视化图表"><rect width="600" height="260" fill="transparent" />{content}</svg></div>;
}
